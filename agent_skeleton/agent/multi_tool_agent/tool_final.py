...
# Session storage for audit progress - use a more stable key system
audit_sessions = {}
...
class AuditSession:
    def __init__(self, session_id: str, category: str):
        ...
        self.status = "started"
        self.state = "waiting_for_question_answer"  # Track conversation state more explicitly
...
def process_chat_message(message: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Check for category selection first - this handles both explicit context and message detection
    categories = get_nist_categories()
    detected_category = None
    
    # Check if category is explicitly provided in context
    if 'category' in context:
        detected_category = context['category']
        print(f"DEBUG: Category from context: {detected_category}")
    
    # If no context category, try to detect from message
    ...
    # If we detected a category, start the session
